...
class MapVeto:
    def __init__(self, name, maps, team_a_id, team_a_name, team_b_id, team_b_name, rules, channel, bot):
        self.name = name
        self.maps = maps[:]
        self.listmaps = maps[:]import json
import os
import asyncio
import discord # type: ignore
from discord.ui import Modal, TextInput, Button, Select, View # type: ignore
    ...
    async def callback(self, interaction: discord.Interaction):
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"`?start_mapveto {self.template_name} {team_a_name} {team_a_id} {team_b_name} {team_b_id}` (Si l'équipe **{team_a_name}** doit démarrer le veto)\n"
                    f"`?start_mapveto {self.template_name} {team_b_name} {team_b_id} {team_a_name} {team_a_id}` (Si l'équipe **{team_b_name}** doit démarrer le veto)"
                ),
                color=discord.Color.blue()
            )
