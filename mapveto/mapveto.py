...
class MapVetoConfig:
    ...
    async def list_vetos(self):
        return [veto['name'] for veto in self.collection.find()]
...
class MapVetoCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.veto_config = MapVetoConfig(self.db)
    ...
    async def mapveto_list(self, ctx):
        ...
        vetos = await self.veto_config.list_vetos()
        if vetos:
            await ctx.send(f"Templates de veto disponibles : {', '.join(vetos)}")
        else:
